...
def train_SVM_poly_kernel(DTR, LTR, C, pi_t, balanced):
    
    Z = numpy.zeros(LTR.shape)
    Z[LTR == 1] = 1
    Z[LTR == 0] = -1
    
    H = (numpy.dot(DTR.T, DTR) + 1)**2 + 1
...
def perform_classification_SVM_poly_kernel(DTR, DTE, LTE, alphaStar, Z):
    k_func = (numpy.dot(DTE.T, DTR) + 1)**2 + 1
    #k_func = (numpy.dot(DTE.T, DTR) + 1)**2
...
def kfold_quadratic_SVM(D, L, k, C, pi_t, balanced):
    idx = range(D.shape[1])
    ...
    for iteration in range(k):
        ...
        LTrain = L[trainIdx]
        w, a, Z = train_SVM_poly_kernel(DTrain, LTrain, C, pi_t, balanced)
        scores, pred = perform_classification_SVM_poly_kernel(DTrain,DTest , LTest, a, Z)
...
def C_tuning(D, L, balanced, filename, datatype):
    ...
    if balanced:
        f = filename + "/quadratic_svm_"+ datatype + "_balanced_"
    else:
        f = filename + "/quadratic_svm_"+ datatype + "_unbalanced_"
    for ef_pi in eff_priors:
        i = 0
        dcfs = []
        for C in Cs:
            print("iteration:", i, "C:", C)
            scores = kfold_quadratic_SVM(D, L, 5, C, 0.5, balanced)
            minDCF = dcf.compute_min_DCF(scores, L, ef_pi, 1, 1)
            print(minDCF)
            dcfs.append(minDCF)
            i+=1
...
def plot_quadratic_svm():
    Cs = numpy.logspace(-3, 0, num=10)
    f_balanced_raw = "../npy_data/quadratic_svm/quadratic_svm_raw_balanced_"
    f_unbalanced_raw = "../npy_data/quadratic_svm/quadratic_svm_raw_unbalanced_"
    f_balanced_gauss = "../npy_data/quadratic_svm/quadratic_svm_gauss_balanced_"
    ...
    axs[0][0].legend()
    fig.tight_layout()
    plt.savefig("../images/quadratic_svm/C_tuning.pdf", format="pdf")
    plt.show()


def compute_last_minDCF(D, L, pi_t, balanced):
    C = 0.1
    D, L = utils.shuffle_dataset(D, L)
    effective_priors = [0.5, 0.1, 0.9]
    dcfs = []
    for ef_pi in effective_priors:
        scores = kfold_quadratic_SVM(D, L, 5, C, pi_t, balanced)
        minDCF = dcf.compute_min_DCF(scores, L, ef_pi, 1, 1)
        dcfs.append(minDCF)
    print(dcfs)
...
def tabular_minDCF():
    ...
    for pi_t in pits:
        print("raw Balanced")
        compute_last_minDCF(D, L, pi_t, True)
        print("gauss Balanced")
        compute_last_minDCF(Y, L, pi_t, True)
    #Unbalanced => 0.5 is useless
    print("raw unbalanced")
    compute_last_minDCF(D, L, 0.5, False)
    print("gauss unbalanced")
    compute_last_minDCF(Y, L, 0.5, False)
...
def train(DTR, LTR, C, pi_t, balanced, filename):
    w = train_SVM_poly_kernel(DTR, LTR, C, pi_t, balanced)
...
def train_models():
    ...
    priors = [0.5,0.1,0.9]
    print("raw unbalanced")
    train(D, L, C, 0.5, False, "../model_parameters/quadratic_svm/w_raw_u")
    print("gauss unbalanced")
    train(Y, L, C, 0.5, False, "../model_parameters/quadratic_svm/w_gauss_u")
    for prior in priors:
        print(prior)
    
        print("raw balanced")
        train(D, L, C, prior, True, "../model_parameters/quadratic_svm/w_raw_b_" + str(prior))
        
        print("gauss balanced")
        train(Y, L, C, prior, True, "../model_parameters/quadratic_svm/w_gauss_b_" + str(prior))

def minDCF_final(D, L, dataset_type, balanced):
    print(dataset_type, balanced)
    if dataset_type == "gauss":
    ...
    if balanced == "b":
        for classifier in classifiers:
            f = "../model_parameters/quadratic_svm/w_" + dataset_type + "_" + balanced + "_" +classifier + ".npy"
            w_tuple = numpy.load(f, allow_pickle=True)
            
            dcfs = []
            for prior in applications:
                scores, lPred = perform_classification_SVM_poly_kernel(DTR, D, L, w_tuple[1], w_tuple[2])
        ...
        f = "../model_parameters/quadratic_svm/w_" + dataset_type + "_" + balanced + ".npy"
        w_tuple = numpy.load(f, allow_pickle=True)
...
if __name__ == "__main__":
    ...
    Y = numpy.load("../npy_data/data/gaussianized_data.npy")
    #TUNING
    C_tuning(D, L, True, "../npy_data/quadratic_svm", "raw")
    C_tuning(D, L, False, "../npy_data/quadratic_svm", "raw")
    C_tuning(Y, L, True, "../npy_data/quadratic_svm", "gauss")
    C_tuning(Y, L, False, "../npy_data/quadratic_svm", "gauss")
    #PLOTTING
    plot_quadratic_svm()
